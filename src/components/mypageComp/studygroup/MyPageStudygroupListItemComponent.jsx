...
function MyPageStudygroupListItemComponent(props) {
    const number = props.number;
    const studygroupInfo = props.studygroupInfo;

    const positionData = props.essentialData.positionData;
    const technologyStackData = props.essentialData.technologyStackData;

    const PositionComponents = () => {
        let amount = studygroupInfo.recruitmentPart.length <= 3 ? studygroupInfo.recruitmentPart.length : 3;
    ...
    function clickUpdateBtn() {
        ...
        window.location.href = "/studygroup/form/"+studygroupInfo.seq;
    }

    function clickDeleteBtn() {
        if(window.confirm("스터디 그룹 모집 글을 삭제하시겠습니까?\n(삭제된 글은 복구할 수 없습니다)")) {
            let opt = {
                data: {
    ...
    return(
        <tr>
            <td>{number}</td>
            <td>{studygroupInfo.title}</td>
            <td>{studygroupInfo.deadlineForRecruitment}</td>
            <td><PositionComponents/></td>
            <td><TechnologyStackComponents /></td>
            <td className='col'>
...
export default MyPageStudygroupListItemComponent;